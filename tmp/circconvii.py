...
class MyCircConv(nn.Module):
    def __init__(self, channels,out_channels=None, adjac):
        super(MyCircConv, self).__init__()
        self.adjac = adjac
        in_channels = channels
        if out_channels is None:
            out_channels = in_channels
        self.convolution = nn.Conv1d(in_channels, out_channels, kernel_size=adjac*2+1)

    def forward(self, x):
        x = torch.cat([x[..., -self.adjac:], x, x[..., :self.adjac]], dim=2)
